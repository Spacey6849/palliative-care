...
export async function POST(req: NextRequest) {
  ...
  const { full_name, location, phone, bin_category } = body;
  ...
  const token = cookieStore.get('bl_session')?.value || cookieStore.get('ecw_session')?.value || cookieStore.get('ecw_admin_session')?.value;
    ...
    const update: Record<string, any> = { full_name: full_name || null, location: location || null, phone: phone || null };
    if (bin_category === 'private' || bin_category === 'public') update.bin_category = bin_category;
