...
export default function ProfilePage() {
  ...
  const [form, setForm] = useState({
    ...
    location: '',
    phone: '',
    bin_category: 'private' as 'private'|'public'
  });
  const [bins, setBins] = useState<any[]>([]);
  const [binsLoading, setBinsLoading] = useState(false);
  const [binMessage, setBinMessage] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (user) {
      setForm({
        ...
        bin_category: ((user as any)?.bin_category === 'public' ? 'public' : 'private')
      });
    }
  }, [user]);
  ...
  const setCategory = (cat: 'private'|'public') => setForm(f => ({ ...f, bin_category: cat }));
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setBinMessage('Failed to load bins');
      } finally {
  ...
  const submitRename = async (id: string) => {
    ...
    setBinMessage(null);
    try {
      const resp = await fetch(`/api/bins/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: renameValue.trim() }) });
      if (!resp.ok) throw new Error();
      setBins(w => w.map(x => x.id === id ? { ...x, name: renameValue.trim() } : x));
      setBinMessage('Bin renamed');
      cancelRename();
    } catch {
      setBinMessage('Rename failed');
    }
  };
  const submitDelete = async (id: string) => {
    ...
    try {
      ...
      if (!resp.ok) throw new Error();
      setBins(w => w.filter(x => x.id !== id));
      setBinMessage('Bin deleted');
    } catch {
      setBinMessage('Delete failed');
    } finally {
      setDeletingId(null);
    }
  };
    ...
    <div className={`min-h-screen w-full relative overflow-hidden ${isDark ? '' : 'bg-gradient-to-br from-gray-50 via-gray-100 to-gray-50'}`}>
      ...
      <div className="mb-10 flex flex-col sm:flex-row sm:items-end gap-6">
        <div>
          ...
          <p className={`mt-2 text-sm ${isDark ? 'text-white/60' : 'text-gray-600'}`}>Manage your account information.</p>
        </div>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <motion.div layout className="lg:col-span-2 space-y-8">
          <div className={`rounded-3xl border backdrop-blur-xl p-8 shadow-xl relative overflow-hidden ${isDark ? 'border-white/15 bg-white/5 shadow-black/40' : 'border-gray-200 bg-white shadow-gray-200/60'}`}>
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                <label className={`text-xs font-medium uppercase tracking-wide ${isDark ? 'text-muted-foreground' : 'text-gray-500'}`}>Bin Preference</label>
                <div className="mt-2 flex gap-3">
                  {(['private','public'] as const).map(opt => (
                    <button type="button" key={opt} disabled={!editing}
                      onClick={()=> setCategory(opt)}
                      className={`px-3 py-1.5 rounded-lg border text-xs font-medium transition disabled:opacity-60 ${form.bin_category===opt? (isDark? 'bg-white/20 border-white/40 text-white':'bg-emerald-600 border-emerald-600 text-white') : (isDark? 'bg-white/10 border-white/20 text-white/60 hover:text-white/80':'bg-white border-gray-300 text-gray-600 hover:bg-gray-50')}`}>
          ...
          <div className={`rounded-3xl border p-8 ${isDark ? 'border-white/10 bg-gradient-to-br from-white/5 to-white/0 backdrop-blur-xl' : 'border-gray-200 bg-white'}`}>
            ...
            <p className={`text-xs mb-4 ${isDark ? 'text-white/60' : 'text-gray-600'}`}>Send yourself a password reset link to your registered email.</p>
            <button onClick={() => window.location.href = '/auth/forgot'} className={`text-xs font-medium transition underline-offset-4 hover:underline ${isDark ? 'text-blue-300 hover:text-blue-200' : 'text-blue-600 hover:text-blue-500'}`}>Reset Password</button>
          </div>
          <div className={`rounded-3xl border p-8 ${isDark ? 'border-white/10 bg-white/5 backdrop-blur-xl' : 'border-gray-200 bg-white'}`}>
            <div className="flex items-center justify-between mb-6">
              <h2 className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>Registered Bins</h2>
              <button onClick={() => {
                // refresh bins
                (async ()=>{
                  setBinsLoading(true); setBinMessage(null);
                  try { const r = await fetch('/api/bins'); const j = await r.json(); setBins(j.bins||[]);} catch { setBinMessage('Reload failed'); } finally { setBinsLoading(false);} })();
            ...
            {binsLoading && <div className={`text-xs mb-4 ${isDark ? 'text-white/60' : 'text-gray-500'}`}>Loading bins...</div>}
            {binMessage && <div className={`text-xs mb-4 font-medium ${/failed/i.test(binMessage) ? (isDark ? 'text-red-300' : 'text-red-600') : (isDark ? 'text-emerald-300' : 'text-emerald-600')}`}>{binMessage}</div>}
            {(!binsLoading && bins.length === 0) && <div className={`text-xs ${isDark ? 'text-white/50' : 'text-gray-500'}`}>No bins registered yet.</div>}
                ...
                <li key={w.id} className={`group rounded-xl border px-4 py-3 flex items-center gap-4 ${isDark ? 'border-white/15 bg-white/5' : 'border-gray-200 bg-gray-50'} transition`}>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className={`mt-0.5 text-[11px] flex flex-wrap gap-2 ${isDark ? 'text-white/50' : 'text-gray-500'}`}>
                      <span>ID: {w.id}</span>
                      {w.bin_type && <span>Type: {w.bin_type}</span>}
                      {w.location?.lat != null && <span>Lat: {Number(w.location.lat).toFixed(4)}</span>}
                      {w.location?.lng != null && <span>Lng: {Number(w.location.lng).toFixed(4)}</span>}
                    </div>
                  </div>
                  {renamingId === w.id ? (
        ...
        <motion.div layout className="space-y-8">
            <div className={`rounded-3xl border p-6 ${isDark ? 'border-white/15 bg-white/5 backdrop-blur-xl' : 'border-gray-200 bg-white'}`}>
              ...
              <ul className={`space-y-3 text-[13px] ${isDark ? 'text-white/70' : 'text-gray-600'}`}>
                <li className="flex justify-between"><span>Role</span><span className={`font-medium ${isDark ? 'text-white/90' : 'text-gray-800'}`}>{role === 'admin' ? 'admin' : 'user'}</span></li>
                <li className="flex justify-between"><span>Bins Created</span><span className={`font-medium ${isDark ? 'text-white/90' : 'text-gray-800'}`}>{bins.length}</span></li>
            ...
            <div className={`rounded-3xl border p-6 ${isDark ? 'border-red-500/30 bg-red-950/30 backdrop-blur-xl' : 'border-red-200 bg-red-50'}`}>
              ...
              <p className={`text-[12px] mb-4 ${isDark ? 'text-red-200/70' : 'text-red-600/80'}`}>Deleting your account will remove all associated bins. This action cannot be undone (feature coming soon).</p>
