// Temporary shim in place for three types during CI; see `project/types/three-shim.d.ts`.
// Remove this comment when a full typing solution (or `@types/three`) is available.
// @ts-nocheck
"use client";
...
export interface LiquidEtherProps {
  ...
  // If true, also listen for mouse/touch events on window so interaction works
  // even when the canvas layer has pointer-events: none.
...
export default function LiquidEther({
  ...
  function makePaletteTexture(stops: string[]): any {
      let arr: string[];
    ...
    class MouseClass {
      ...
      init(container: HTMLElement) {
        ...
        if (this.listenOnDocument) {
          window.addEventListener('mousemove', this._onMouseMove, { passive: true } as any);
          window.addEventListener('touchmove', this._onTouchMove as any, { passive: true } as any);
          window.addEventListener('touchstart', this._onTouchStart as any, { passive: true } as any);
        }
      }
      dispose() {
  ...
  Mouse.listenOnDocument = !!listenOnDocument;
...
`;
    ...
    class Simulation {
      ...
      fbos: Record<string, THREE.WebGLRenderTarget | null> = {
        vel_0: null, vel_1: null, vel_viscous0: null, vel_viscous1: null, div: null, pressure_0: null, pressure_1: null
      };
      ...
      calcSize() { const width = Math.max(1, Math.round(this.options.resolution * Common.width)); const height = Math.max(1, Math.round(this.options.resolution * Common.height)); this.cellScale.set(1 / width, 1 / height); this.fboSize.set(width, height); }
  ...
  Mouse.init(props.$wrapper);
        // debug: indicate manager init
        try { console.debug('[LiquidEther] WebGLManager: initializing', { wrapper: props.$wrapper }); } catch (e) {}
      ...
      dispose() {
        try {
          ...
          Mouse.dispose();
          if (Common.renderer) { const canvas = Common.renderer.domElement; if (canvas && canvas.parentNode) canvas.parentNode.removeChild(canvas); Common.renderer.dispose(); }
        } catch {}
      }
    }
    ...
    return () => {
      ...
      if (resizeObserverRef.current) { try { resizeObserverRef.current.disconnect(); } catch {} }
      if (intersectionObserverRef.current) { try { intersectionObserverRef.current.disconnect(); } catch {} }
      if (webglRef.current) { webglRef.current.dispose(); }
      webglRef.current = null;
    };
  }, [
    BFECC, cursorSize, dt, isBounce, isViscous, iterationsPoisson, iterationsViscous, mouseForce, resolution, viscous, colors,
    autoDemo, autoSpeed, autoIntensity, takeoverDuration, autoResumeDelay, autoRampDuration, listenOnDocument
  ]);
  ...
  return (
    <div ref={mountRef} className={`w-full h-full relative overflow-hidden pointer-events-none touch-none ${className || ''}`} style={style} />
  );
}
